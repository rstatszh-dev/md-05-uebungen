...
Nutze den `gapminder` Datensatz um mittels bedingter Anweisungen eine neue Spalte zu erstellen.

1.  Starte mit `gapminder` und nutze den Pipe-Operator `|>` um die folgenden Schritte zu verketten.

2.  Nutze die Funktion `mutate()`, um eine neue Spalte `life_expectancy_category` zu erstellen welche mittels `case_when()` die Lebenserwartung eines Landes in die Kategorien "Sehr niedrig", "Niedrig", "Mittel", "Hoch" und "Sehr hoch" einteilt.

3.  Nutze die folgenden Bedingungen:
...
4.  Speichere den resultierenden Datensatz in einem neuen Daten-Objekt `gapminder_cat`.
